...
// [start, end]
void shift_down_v2(vector<int>& arr, int begin, int start, int end) {
    ...
    if (idx != start) {
        swap(arr[idx], arr[start]);
        // 递归调整其他不满足堆性质的部分
        shift_down_v2(arr, begin, idx, end);
    }
}

void hs_v2(vector<int>& arr, int begin, int end) {
    // 1. 建最大堆
    // 从最后一个非叶结点开始, 对每一个非叶结点进行堆调整
    int last = begin + (end - begin - 1) / 2;
    for (int i = last; i >= begin; i--) {
        shift_down_v2(arr, begin, i, end);
    }
    // 2. 排序
    for (int i = end; i >= begin + 1; i--) {
        // 将当前最大的放置到数组末尾
        swap(arr[begin], arr[i]);
        shift_down_v2(arr, begin, begin, i - 1);
        int e = 3;
    }
}
